...
type GenerateCardsFromDOCXRequest struct {
...
func (h *AIHandler) GenerateCardsFromDOCX(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			...
			"error_details": []gin.H{{"field": "file", "message": "DOCX file is required"}},
		})
		return
	}
	defer file.Close()
	...
	setName := c.DefaultPostForm("set_name", "DOCX Generated Set")
	setDescription := c.PostForm("set_description")

	docxService := services.NewDOCXService()
	text, err := docxService.ExtractTextFromReader(file)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error_type":    "validation_failed",
			"error_message": "Failed to process DOCX",
	...
	if len(text) < 100 {
		c.JSON(http.StatusBadRequest, gin.H{
			"error_type":    "validation_failed",
			"error_message": "DOCX contains insufficient text",
			"error_details": []gin.H{{"field": "file", "message": "DOCX must contain at least 100 characters of text"}},
		})
		return
	}
