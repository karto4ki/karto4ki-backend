...
type GenerateCardsFromPDFRequest struct {
...
func (h *AIHandler) GenerateCardsFromPDF(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			...
			"error_details": []gin.H{{"field": "file", "message": "PDF file is required"}},
		})
		return
	}
	defer file.Close()
	...
	setName := c.DefaultPostForm("set_name", "PDF Generated Set")
	setDescription := c.PostForm("set_description")

	pdfService := services.NewPDFService()
	text, err := pdfService.ExtractTextFromReader(file)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error_type":    "validation_failed",
	...
	if len(text) < 100 {
		c.JSON(http.StatusBadRequest, gin.H{
			"error_type":    "validation_failed",
			"error_message": "PDF contains insufficient text",
			"error_details": []gin.H{{"field": "file", "message": "PDF must contain at least 100 characters of text"}},
		})
		return
	}

	resp, err := h.service.GenerateCards(c.Request.Context(), userID, services.GenerateCardsRequest{
		Text:           text,
		CardCount:      cardCount,
